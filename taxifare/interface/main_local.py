...
def preprocess_and_train(min_date:str = '2009-01-01', max_date:str = '2015-01-01') -> None:
    ...
    history = train_model(model, X_train_processed, y_train, batch_size=batch_size, patience=patience)

    # Compute the validation metric (min val_mae) of the holdout set
    val_mae = np.min(history[1].history['val_mae'])
...
def preprocess(min_date: str = '2009-01-01', max_date: str = '2015-01-01') -> None:
    ...
    if data_query_cache_exists:
        ...
        # YOUR CODE HERE
        chunks = bigquery.Client(project=GCP_PROJECT).query(query).result(page_size=CHUNK_SIZE).to_dataframe_iterable() #in one line
        #client = bigquery.Client(project=GCP_PROJECT)
        #query_job = client.query(query)
        #result = query_job.result(page_size=CHUNK_SIZE)
        #chunk = result.to_dataframe_iterable()
    ...
    for chunk_id, chunk in enumerate(chunks):
        ...
        X_chunk_processed = preprocess_features(X_chunk)
        chunk_processed = pd.DataFrame(np.concatenate((X_chunk_processed, y_chunk), axis=1))
...
def train(min_date:str = '2009-01-01', max_date:str = '2015-01-01') -> None:
    ...
    for chunk_id, chunk in enumerate(chunks):
        ...
        metric_val_chunk = np.min(history.history['val_mae'])
        metrics_val_list.append(metric_val_chunk)
